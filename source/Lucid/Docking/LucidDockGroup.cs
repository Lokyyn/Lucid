...
[ToolboxItem(false)]
public class LucidDockGroup : Panel
{
    #region Field Region

    private List<LucidDockContent> _contents = new List<LucidDockContent>();

    private Dictionary<LucidDockContent, LucidDockTab> _tabs = new Dictionary<LucidDockContent, LucidDockTab>();

    private LucidDockTabArea _tabArea;

    private LucidDockTab _dragTab = null;
    ...
    public LucidDockRegion DockRegion { get; private set; }

    public LucidDockArea DockArea { get; private set; }

    public LucidDockContent VisibleContent { get; private set; }

    public int Order { get; set; }

    public int ContentCount { get { return _contents.Count; } }

    private ToolTip TT_Dock = new ToolTip();

    #endregion

    #region Constructor Region

    public LucidDockGroup(LucidDockPanel dockPanel, LucidDockRegion dockRegion, int order)
    {
        ...
        _tabArea = new LucidDockTabArea(DockArea);
    ...
    public void AddContent(LucidDockContent dockContent)
    {
        ...
        var newOrder = dockContent.InsertOrder.HasValue ? dockContent.InsertOrder.Value : -1;

        if (_contents.Count > 0)
        {
            // Custom Order for existing tabs

            if (newOrder >= 0)
            {
                foreach (var otherContent in _contents)
                {
                    if (otherContent.Order >= newOrder)
                        otherContent.Order += 1;
                }
            }
            else
            {
                ...
                {
                    if (otherContent.Order >= newOrder)
                        newOrder = otherContent.Order + 1;
                }
            }

            dockContent.Order = newOrder;
        }
        ...
        _tabs.Add(dockContent, new LucidDockTab(dockContent));
    ...
    private void UpdateTabArea()
    {
        ...
        if (DockArea == LucidDockArea.Document)
        {
            ...
            if (_tabArea.DockSettingsVisible)
            {
                _tabArea.DockSettingsRectangle = new Rectangle(_tabArea.ClientRectangle.Right - dropdownSize, 0, dropdownSize, dropdownSize);
                _tabArea.DropdownRectangle = new Rectangle(_tabArea.ClientRectangle.Right - _tabArea.DockSettingsRectangle.Width - dropdownSize, 0, dropdownSize, dropdownSize);
            }
            else
    ...
    private void BuildTabs()
    {
        ...
        var closeButtonSize = ThemeProvider.Theme.Type == ThemeType.Dark ? DockIcons.close_white_12.Width : DockIcons.close_dark_12.Width;
        ...
        foreach (var content in orderedContent)
        {
            ...
            var y = DockArea == LucidDockArea.Document ? 0 : ClientRectangle.Height - ThemeProvider.Theme.Sizes.ToolWindowTabAreaSize;
            var height = DockArea == LucidDockArea.Document ? ThemeProvider.Theme.Sizes.DocumentTabAreaSize : ThemeProvider.Theme.Sizes.ToolWindowTabAreaSize;
    ...
    public void SetVisibleContent(LucidDockContent content)
    {
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        else if (_tabArea.DockSettingsVisible && _tabArea.DockSettingsRectangle.Contains(e.Location))
        {
            _tabArea.DockSettingsHot = true;
        ...
        _tabArea.DockSettingsHot = false;
        ...
        foreach (var tab in _tabs.Values)
        {
            ...
            if (hot && !string.IsNullOrEmpty(tab.DockContent.DockText) && tab.DockContent.DockText.Contains("…"))
                TT_Dock.Show(tab.DockContent.DockTextOriginal, tab.DockContent, e.Location);
            else
                TT_Dock.Hide(tab.DockContent);
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        else if (_tabArea.DockSettingsRectangle.Contains(e.Location))
        {
            _tabArea.DockSettingsHot = true;
            return;
        }
        ...
        foreach (var tab in _tabs.Values)
        {
            ...
            if (rect.Contains(e.Location))
            {
                if (e.Button == MouseButtons.Middle)
                {
                    ...
                    tab.DockContent.Dispose();
                    tab.DockContent = null;

                    if (_contents.Count > 0)
                    {
                        var highestOrder = _contents.Max(u => u.Order);
                        var nextContentInOrder = _contents.FirstOrDefault(u => u.Order == highestOrder);

                        if (nextContentInOrder != null)
                            nextContentInOrder.Select();
                    }
    ...
    protected override void OnMouseUp(MouseEventArgs e)
    {
        ...
        {
            if (_tabArea.DockSettingsHot)
                _tabArea.ShowDockSettingsMenu(this, new Point(_tabArea.DockSettingsRectangle.Left, _tabArea.DockSettingsRectangle.Bottom - 2));
        }
    ...
    private void TabMenuItem_Select(object sender, EventArgs e)
    {
        ...
        var content = menuItem.Tag as LucidDockContent;
        if (content == null)
            return;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        if (DockArea == LucidDockArea.Document)
        {
            ...
            if (_tabArea.DockSettingsVisible)
            {
                ...
                using (var img = DockIcons.settings)
                {
                    g.DrawImageUnscaled(img, dockSettingsRect.Left + (dockSettingsRect.Width / 2) - (img.Width / 2), dockSettingsRect.Top + (dockSettingsRect.Height / 2) - (img.Height / 2) + 1);
                }
            }
        }
    }

    private void PaintDocumentTab(Graphics g, LucidDockTab tab)
    {
        ...
        // Draw text
        var textColor = isVisibleTab ? Helper.ColorExtender.GetContrastColor(ThemeProvider.Theme.Colors.Accent) : ThemeProvider.Theme.Colors.TextPrimary;
        ...
        var img = tab.CloseButtonHot ? icoSelected : icoUnselected;

        if (isVisibleTab)
        {
            if (isActiveGroup)
                img = tab.CloseButtonHot ? icoSelected : icoUnselected;
            else
                img = tab.CloseButtonHot ? icoSelected : icoUnselected; //DockIcons.active_inactive_close; todo: maybe add icon later?
        }

        var closeRect = RectangleToTabArea(tab.CloseButtonRectangle);
        g.DrawImageUnscaled(img, closeRect.Left, closeRect.Top + 1);
    }

    private void PaintToolWindowTab(Graphics g, LucidDockTab tab)
    {
        ...
        var textColor = isVisibleTab ? ThemeProvider.Theme.Colors.Accent : ThemeProvider.Theme.Colors.TextPrimary;
        using (var b = new SolidBrush(textColor))
        {
