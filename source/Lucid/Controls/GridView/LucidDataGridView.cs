...
namespace Lucid.Controls.GridView;

public class LucidDataGridView : UserControl, ISupportInitialize
{
    ...
    public LucidDataGridView()
    {
        ...
        // Apply main accent color to complete header
        _base.CellPainting += _base_CellPainting;
        _base.CellFormatting += _base_CellFormatting;
        ...
        // Add listener to theme changed
        Theming.ThemeProvider.OnThemeChanged += ThemeProvider_OnThemeChanged;
    }
        ...
        if (e.RowIndex == -1 && e.ColumnIndex > -1)
        {
            using (var b = new SolidBrush(BackColor))
            {
                ...
                TextRenderer.DrawText(e.Graphics, string.Format("{0}", e.FormattedValue),
                    e.CellStyle.Font, e.CellBounds, textColor,
    ...
    public bool SelectRowByLocation(Point location)
    {
        location = PointToClient(location);

        var ht = HitTest(location.X, location.Y);

        if (ht != null && ht.RowIndex != -1)
        {
            _base.ClearSelection();
            _base.Rows[ht.RowIndex].Selected = true;
            _base.CurrentCell = _base.Rows[ht.RowIndex].Cells[ht.ColumnIndex];
            //_base.Rows[ht.RowIndex].Cells[ht.ColumnIndex].Selected = true;
    ...
    {
        _base.DefaultCellStyle = GetCellStyle(true, false, false);
        _base.AlternatingRowsDefaultCellStyle = GetCellStyle(true, true, false);
    }
    ...
    private void BaseCellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
    {
        ...
        // Invoke event
        var args = new LucidDataGridViewSafeCellFormattingEventArgs(e.ColumnIndex, e.RowIndex, e.Value, e.DesiredType, e.CellStyle, column, row);
    ...
    public event LucidDataGridViewSafeCellFormattingEventHandler CellFormattingSafe;
    ...
    public event DataGridViewCellEventHandler CelLucidick { add { _base.CellClick += value; } remove { _base.CellClick -= value; } }
